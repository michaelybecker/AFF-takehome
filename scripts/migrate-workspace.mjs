...
if(existing){ console.log('Shared workspace verified:',{records:Object.keys(existing.value.records).length,media:Object.keys(existing.value.files).length,removed:existing.value.records['explorations.json']?.removed.length});process.exit(0);}
...
console.log('Migration verified:',result,'Open demo access is enabled.');
