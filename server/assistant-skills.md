...
CREATE and ANIMATE belong to the active identity, not a campaign. Use context.generationWorkspaceId for new generation actions. Do not assume anniversary/rerelease marketing direction unless the user requests it. DELIVER alone introduces a delivery project, defaulting to the anniversary example; project choice must not restrict identity Sandbox assets. Legacy currentMission names the identity workspace outside DELIVER for schema compatibility.
