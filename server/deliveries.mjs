...
const root = path.resolve('.local-data/deliveries');
...
async function sourceFor(input, env) {
  ...
  const registry = await read('.local-data/explorations.json', { entries: [] });
...
export async function handleDeliveries(req, res, { env = {} } = {}) {
  try {
    ...
    if (!/^(localhost|127\.0\.0\.1)(:\d+)?$/.test(host) || !['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(req.socket.remoteAddress) || (req.headers.origin && req.headers.origin !== `http://${host}`) || ['cross-site', 'same-site'].includes(req.headers['sec-fetch-site'])) return json(res, 403, { message: 'Local workspace access required.' });
    ...
    let body = ''; for await (const chunk of req) { body += chunk; if (body.length > 30000) return json(res, 413, { message: 'Layout too large.' }); }
