...
import { Expand, Maximize2, Pause, Play, Volume2, VolumeX, X, ZoomIn, ZoomOut } from 'lucide-react';
import { clipTitle, clock, frameSrc, passNames, passes, type Canon, type Clip, type Pass } from './data';
...
export function Modal({ title, subtitle, children, onClose, wide = false }: { title: string; subtitle?: string; children: ReactNode; onClose: () => void; wide?: boolean }) {
  ...
  return <dialog ref={dialog} aria-label={title} className={`modal ${wide ? 'wide' : ''}`} onCancel={e => { e.preventDefault(); onClose(); }} onClick={e => { if (e.target === dialog.current) { const r = dialog.current!.getBoundingClientRect(); if (e.clientX < r.left || e.clientX > r.right || e.clientY < r.top || e.clientY > r.bottom) onClose(); } }}>
    <header className="modal-heading"><div><span className="eyebrow">MARVEL STUDIOS / CONTENT STUDIO</span><h2>{title}</h2>{subtitle && <p>{subtitle}</p>}</div><IconButton label="Close viewer" onClick={onClose}><X /></IconButton></header>
    {children}
  </dialog>;
}

export function VideoTile({ clip, onOpen, selected = false, onSelect }: { clip: Clip; onOpen: (clip: Clip) => void; selected?: boolean; onSelect?: () => void }) {
  ...
  return <article className={`media-tile ${selected ? 'selected' : ''}`}>
    ...
    <div className="tile-caption"><strong>{clipTitle(clip)}</strong><span>{clip.id.startsWith('Hero') ? 'Campaign candidate' : /Nano|Mask/.test(clip.id) ? 'Variant reference' : 'Production reference'}</span></div>
...
export function VideoPlayer({ clip, autoPlay = false, onExpand }: { clip: Clip; autoPlay?: boolean; onExpand?: () => void }) {
  ...
  return <div className="player" ref={root}>
    ...
    <div className="player-controls"><IconButton label={playing ? 'Pause' : 'Play'} onClick={toggle}>{playing ? <Pause /> : <Play />}</IconButton><span className="timecode">{clock(time)}</span><input aria-label="Playback position" type="range" min="0" max={duration || 1} step="0.01" value={time} onChange={e => { const t = Number(e.target.value); video.current!.currentTime = t; setTime(t); }} /><span className="timecode">{clock(duration)}</span>
      ...
      <IconButton label="Fullscreen" onClick={() => { if (document.fullscreenElement) document.exitFullscreen().catch(() => {}); else root.current?.requestFullscreen().catch(() => {}); }}><Maximize2 /></IconButton>
    </div>
  </div>;
}
...
export function ImageViewer({ item }: { item: Canon }) {
  const [zoom, setZoom] = useState(1);
  return <><div className="image-stage"><img src={item.src} alt={item.title} style={{ width: `${zoom * 100}%`, maxWidth: zoom === 1 ? '100%' : 'none', maxHeight: zoom === 1 ? '65vh' : 'none' }} /></div><div className="player-controls"><span>{item.variant}</span><span className="spacer" /><IconButton label="Zoom out" disabled={zoom <= 1} onClick={() => setZoom(Math.max(1, zoom - .5))}><ZoomOut /></IconButton><span>{Math.round(zoom * 100)}%</span><IconButton label="Zoom in" disabled={zoom >= 3} onClick={() => setZoom(Math.min(3, zoom + .5))}><ZoomIn /></IconButton></div></>;
}
