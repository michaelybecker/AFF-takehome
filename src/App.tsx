...
function readRoute() { const hash = location.hash; return hash.startsWith('#/activate') ? { page: 'activate', mode: (hash === '#/activate/adapt' ? 'adapt' : hash === '#/activate/animate' ? 'animate' : 'create') as Mode } : { page: 'identity', mode: 'create' as Mode }; }
...
export default function App() {
  ...
  useEffect(() => { const changed = () => { if (location.hash.startsWith('#/activate') && !['#/activate/create', '#/activate/animate', '#/activate/adapt'].includes(location.hash)) history.replaceState(null, '', '#/activate/create'); setRoute(readRoute()); window.scrollTo(0, 0); }; changed(); window.addEventListener('hashchange', changed); return () => window.removeEventListener('hashchange', changed); }, []);
  ...
  return <>
    <header className="app-header">
      <a className="brand-lockup" href="#/identity" aria-label="Generative Identity Kit home"><img src="/media/brand/marvel-studios.jpg" alt="Marvel Studios" /><span className="brand-divider" /><strong>Generative Identity Kit</strong></a>
      <nav className="primary-nav" aria-label="Main navigation"><a href="#/identity" aria-current={route.page === 'identity' ? 'page' : undefined} className={route.page === 'identity' ? 'active' : ''}>IDENTITY</a>{(['create', 'animate', 'adapt'] as Mode[]).map(m => <a key={m} href={'#/activate/' + m} aria-current={route.page === 'activate' && route.mode === m ? 'page' : undefined} className={route.page === 'activate' && route.mode === m ? 'active' : ''}>{m.toUpperCase()}</a>)}</nav>
      <div className="powered"><img src="/media/brand/firefly.svg" alt="" /><span>Powered by <strong>Firefly Foundry</strong></span></div>
      <IconButton label="About Generative Identity Kit" onClick={() => { setAccountOpen(false); setViewer({ type: 'about' }); }}><CircleHelp /></IconButton>
      <div className="account-menu-wrap">
        ...
        {accountOpen && <>
          ...
          <section className="account-popover" role="dialog" aria-label="Michael Becker account">
            ...
            <span className="account-section-label">GENERATIVE IDENTITY KIT</span>
...
<section id="derived"><SectionHeading number="05" title="Generative Identity" subtitle="Learned identity and generated sample results" /><DerivedIdentity record={manifest.derived} /></section>
      <footer className="page-footer"><span>Marvel Studios <span className="muted-dot">/</span> Generative Identity Kit</span><span>Identity kit v{manifest.version}</span></footer>
    </>}</main>
    ...
    {viewer?.type === 'about' && <Modal title="Generative Identity Kit" subtitle="Powered by Firefly Foundry" onClose={() => setViewer(null)}><div className="modal-copy"><p>Interview case-study prototype. Hypothetical engagement for The Walt Disney Company, piloted with Marvel Studios.</p><p>The signed-in local profile can use live still and motion services while their configured providers are available. The Krea identity adaptation is trained and selected for CREATE; generated drafts still require creative review.</p><p>Production footage and design references are demonstration material; their inclusion does not imply studio approval. Alternate armor references remain labeled for review.</p><p className="muted">Neutral studio lighting: Studio Small 09, Poly Haven (CC0).</p><ConnectionCheck /></div></Modal>}
  </>;
}
