import { identityWorkspaceId } from './missions';
import { assistantPayload } from './assistantContext';
...
export default function StudioAssistant({ manifest, route }: { manifest: Manifest; route: string }) {
  ...
  const mission = missions.missions.find(m => m.id === (route === 'deliver' ? missions.selectedId : identityWorkspaceId))!;
  ...
  async function send(reviewAsset: ReviewAsset | undefined = attached) {
    ...
    try {
      ...
      const response = await fetch('/api/assistant', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json', 'X-Content-Studio': '1' }, signal: AbortSignal.timeout(90000),
        body: JSON.stringify({ ...(evidence ? { review: evidence } : {}), ...assistantPayload({ ...studioContext(manifest, route), capabilities: live, pendingProposals: proposals.filter(p => p.state === 'pending').map(p => ({ name: p.action.name, arguments: p.action.arguments })), executionReceipts: proposals.filter(p => p.state !== 'pending').slice(-12).map(p => ({ actionId: p.action.id, name: p.action.name, status: p.state, message: p.detail })) }, next, reviewAsset?.id) }) });
      ...
      const actions: AssistantAction[] = result.actions.slice(0, 4).filter((a: AssistantAction) => a && typeof a.id === 'string' && labels[a.name] && a.arguments && typeof a.arguments === 'object').map((a: AssistantAction) => {
        ...
        return { ...a, id: crypto.randomUUID(), arguments: a.name === 'generate_motion' ? { ...a.arguments, missionId: identityWorkspaceId, mode: 'image-to-video', sourceId, referenceIds: [] } : a.name === 'generate_still' ? { ...a.arguments, missionId: identityWorkspaceId } : a.arguments };
      });
  ...
  return <aside className={`studio-assistant ${open ? 'is-open' : 'is-closed'} ${expanded ? 'is-expanded' : ''}`} aria-label="AI Assistant" onDragOver={event => { if (acceptAssetDrag(event)) setOpen(true); }} onDrop={event => { if (!acceptAssetDrag(event)) return; const id = event.dataTransfer.getData(assetMime); if (context.availableAssets.some(asset => asset.id === id)) { setReviewId(id); setOpen(true); } }}>
    ...
    {expanded && route === 'deliver' && <nav className="assistant-missions" aria-label="Assistant projects"><span className="eyebrow">PROJECTS</span>{missions.missions.filter(item => item.id !== identityWorkspaceId).map(item => <button key={item.id} className={item.id === missions.selectedId ? 'selected' : ''} disabled={busy || running} onClick={() => setSelectedMission(item.id)}>{item.title}</button>)}<a href="#/activate/create" onClick={() => setExpanded(false)}><PanelsTopLeft size={15} />Detailed workspace</a></nav>}
    {open && <><div className="assistant-context"><img src="/media/stills/studio-hero-portrait.webp" alt="Iron Man Mark III" /><div><strong>Iron Man / Mark III</strong><span>{mission.title}</span><small>{route === 'identity' ? 'Identity' : route === 'create' ? 'Create' : route === 'animate' ? 'Animate' : 'Deliver'} · Current workspace</small></div></div>
    <details className="assistant-context-details"><summary>Context and connection</summary><p>{mission.summary}</p><p>{route === 'deliver' ? 'Project' : 'Identity workspace'}: {mission.id}</p><p>Still master: {mission.draft.stillId || 'None'}<br />Motion master: {mission.draft.motionId || 'None'}</p><p>{context.availableAssets.length} identity assets · Stills {health?.still.available ? 'available' : 'unavailable'} · Motion {health?.motion.available ? 'available' : 'unavailable'}</p><p>Prototype assistant{status.model ? ` · ${status.model}` : ''}. Uses this session conversation and curated studio instructions. Separate from Codex and Adobe AI Assistant.</p></details>
