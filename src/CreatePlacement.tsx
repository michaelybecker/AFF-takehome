...
export default function CreatePlacement({ campaignId, onCreate, onClose }: { campaignId: string; onCreate: (p: CustomPlacement) => void; onClose: () => void }) {
  ...
  return <Modal title="Create placement" subtitle="Define a delivery format for this project" onClose={onClose}><form className="mission-editor" onSubmit={event => {
    ...
    {error && <p role="alert">{error}</p>}<div className="asset-export-actions"><button className="button secondary" type="button" onClick={onClose}>Cancel</button><button className="button primary" type="submit">Create placement</button></div>
  </form></Modal>;
}
